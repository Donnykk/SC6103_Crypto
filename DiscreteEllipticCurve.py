...
from tools import egcd, bits, pow_mod, positive_mod, cipolla_alg
...
class DiscreteEllipticCurve(object):
    ...
    def __gen_x_y(self):
        ...
        for x in range(0, self.p):
            ...
            if pow_mod(y_double, int((self.p - 1) / 2), self.p) == 1:
    ...
    def draw_curve(self, xlim=None, ylim=None, save=False):
        ...
        if save == False:
            ...
            plt.savefig(
                "../picture/discreteEC_{a}_{b}_{p}_{time}.png".format(
                    a=self.a,
                    b=self.b,
                    p=self.p,
    ...
    def __check_on_curve(self, point):
        return (
            True
            if (point[0] ** 3 + self.a * point[0] + self.b - point[1] ** 2) % self.p
            == 0
            else False
        )
    ...
    def __cal_R(self, P, Q, solve):
        ...
        return positive_mod(solve[0][0], self.p), positive_mod(
            self.p - solve[0][1], self.p
        )
    ...
    def get_scalar_multiplication(self, n, P):
        ...
        if n == 0:
            return 0, 0
        if n < 0:
            return self.get_scalar_multiplication(-n, (P[0], -P[1] % self.p))
        if self.know_GF:
            n = n % self.GF
            self.R = (0, 0)
            self.P = P
    ...
    def fast_GF(self, P):
        ...
        if self.p > 100000:
            for GF in __import__("tqdm").tqdm(range(self.p - 100000, self.p)):
                if self.get_scalar_multiplication(
                    1, self.P
                ) == self.get_scalar_multiplication(1 + GF, self.P):
                    self.get_GF(GF)
                    return True
        return False

    def __str__(self):
        return "$$ y^2 \equiv x^3 + {a}x + {b} (mod \;{p})$$".format(
